...
function renderEpisodes(animate) {
    ...
    list.innerHTML = eps.map(ep => {
        const thumbStyle = ep.thumb
            ...
            : `background:linear-gradient(135deg,#0a1628,#2a1a00)`;
...
function playEpisode(seasonIdx, epNum, animate = false) {
    ...
    // Configurar botones
    const IS_MOVIE_UI = (currentEpisode.type === 'movie' || (typeof SERIE !== 'undefined' && (SERIE.type === 'movie' || SERIE.type === 'pelicula')));
    const pFooter = $('player-footer');
...
function closePlayer() {
    if (typeof SERIE !== 'undefined' && (SERIE.type === 'pelicula' || SERIE.type === 'movie')) {
...
function proxyFetch(url, timeoutMs) {
    ...
    // Si la URL es Goodstream, el token de video que genera el servidor ('t=...') está atado a la IP que hace la petición.
    // NUNCA podemos usar CORS_PROXIES para Goodstream, porque el token quedará atado a la IP del proxy (ej. AllOrigins/Cloudflare) 
    // y al intentar reproducir localmente, dará 403 Forbidden. SE DEBE hacer directo para que pida la IP del usuario.
    const isIPBound = url.includes('goodstream');

    const tryProxy = (idx) => {
        if (isIPBound) return Promise.reject(new Error('Proxy bypass forced due to IP-Bound tokens'));
    ...
    if (!GLOBAL_IS_MOBILE || isIPBound) {
        console.log('WOLF_INTERCEPT_URL:', url);
        console.log('🚀 [Direct Fetch] Desktop detected, trying direct fetch:', url);
        ...
        if (window.ipcRenderer) {
            return new Promise((resolve) => {
                ...
                setTimeout(() => {
                    ipcRenderer.removeListener('proxy-response', handler);
                    fetchDirect(url, timeoutMs).then(resolve).catch(() => {
                        console.warn('⚠️ Electron fallback failed, trying proxies...');
                        tryProxy(0).then(resolve).catch(() => resolve({ contents: '' }));
                    });
                }, 10000);
            });
        }

        return fetchDirect(url, timeoutMs).catch(e => {
            console.warn('⚠️ Direct fetch failed (CORS?), trying proxies...', e.message);
            return tryProxy(0);
        });
    }
...
function extractVideoUrl(code) {
    ...
    // Try to parse goodstream/jwplayer sources array (Priority 1)
    const sourcesMatch = code.match(/sources\s*:\s*\[\s*\{[\s\S]*?file\s*:\s*["']([^"']+)["']/i);
    if (sourcesMatch && sourcesMatch[1] && sourcesMatch[1].includes('.m3u8')) {
...
function resolveUrl(server) {
    ...
    // Si estamos en PC (navegador local sin Capacitor) y es Goodstream, ignoramos la extracción de M3U8 
    // porque el navegador siempre bloqueará el video por CORS/Referer, arrojando errores rojos.
    // Es más limpio montar el iframe directamente.
    if (!GLOBAL_IS_MOBILE && url.includes('goodstream')) {
            ...
            if (v) {
                ...
                v.addEventListener('error', (e) => {
                    ...
                    wrap.innerHTML = `<div class="player-placeholder">
                      ...
                      <small>El servidor rechazó la conexión. Intenta con otro servidor.</small>
                    </div>`;
                });
...
// ── Inicialización ────────────────────────────────────────
const IS_WHOLE_MOVIE = typeof SERIE !== 'undefined' && (SERIE.type === 'pelicula' || SERIE.type === 'movie');

if (IS_WHOLE_MOVIE) {
    // Ocultar UI de serie inmediatamente para evitar parpadeos
    const sHeader = $('serie-header');
    if (sHeader) sHeader.style.display = 'none';
    const sWrap = document.querySelector('.seasons-wrap');
    if (sWrap) sWrap.style.display = 'none';
    const eList = $('episodes-list');
    if (eList) eList.style.display = 'none';

    const firstEp = SERIE.seasons?.[0]?.episodes?.[0];
    if (firstEp) {
        // Ejecutar playEpisode con un delay mínimo para asegurar que otros inits terminen
        setTimeout(() => playEpisode(0, firstEp.num), 30);
    }
} else {
    renderTabs();
    renderEpisodes(true);
